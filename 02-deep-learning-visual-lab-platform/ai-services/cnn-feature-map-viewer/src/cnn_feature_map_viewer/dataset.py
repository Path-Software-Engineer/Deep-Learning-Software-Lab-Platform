"""Deterministic extraction and split contract for the official sprite."""
...
def stratified_splits(
    ...
    *,
    ...
    for label_index in range(len(CLASS_NAMES)):
        class_samples = [sample for sample in samples if sample.label_index == label_index]
        permutation = torch.randperm(len(class_samples), generator=generator).tolist()
        ordered = [class_samples[index] for index in permutation]
        train_end = configuration.train_per_class
        validation_end = train_end + configuration.validation_per_class
        test_end = validation_end + configuration.test_per_class
...
def normalize(pixels: torch.Tensor) -> torch.Tensor:
    configuration = PREPROCESSING_CONFIGURATION
    return (pixels - configuration.mean) / configuration.std
...
def tensor_dataset(
    ...
    *,
    ...
    for sample in samples:
        images.append(normalize(sample.pixels))
        labels.append(sample.label_index)
        if augment:
            for _ in range(TRAINING_CONFIGURATION.augmentation_copies):
                images.append(normalize(_augmented_copy(sample.pixels, generator=generator)))
                labels.append(sample.label_index)
